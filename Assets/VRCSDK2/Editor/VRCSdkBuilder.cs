...
public class VRCSdkBuilder : MonoBehaviour 
{
	...
	static string recentlySavedPath;
	...
	[MenuItem("VRChat/Build and Run Scene from Selection")]
	static void ExportSceneResourceAndRun()
	{
		ExportSceneResource();

		string file = WWW.EscapeURL(recentlySavedPath);
		string url = "vrchat://create?hidden=true&name=BuildAndRun&url=file:///" + file;
		Process.Start( url );
	}
